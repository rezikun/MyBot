namespace Recommendations
{
    ...
    public class RecommendationsApiWrapper
    {
        ...
        public void AddUsageEvent(string modelId, string WhatHappened)
        {
            string uri = BaseUri + "/models/" + modelId + "/usage/events?";
            
            byte[] byteData = Encoding.UTF8.GetBytes(WhatHappened);
        ...
        public void DeleteUsageFiles(string modelId)
        {
            string uri = BaseUri + "/models/" + modelId + "/usage?";
            var response = _httpClient.DeleteAsync(uri).Result;
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception(
                    String.Format("Error {0}: Failed to get user recommendations for modelId {1}, Reason: {2}",
                    response.StatusCode, modelId, ExtractErrorInfo(response)));
            }
        }
        /// <summary>
